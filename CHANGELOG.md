...
- First-class OpenTelemetry instrumentation (`Xyo.Sdk` `ActivitySource` and `Meter`): a client span per public operation with span status derived from the typed exception hierarchy, request/duration/rate-limit/redirect-refused/download-bound-tripped metrics, and an optional `ILoggerFactory` on `XyoClientConfig`. Inert (no allocation) with no listener registered, and never emits the API key.
